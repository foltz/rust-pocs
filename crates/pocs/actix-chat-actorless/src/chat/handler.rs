...
pub async fn ws_message_handler(
        ...
        match select(messages, tick).await {
            ...
            Either::Left((Either::Left((Some(Ok(msg)), _)), _)) => {
                ...
                match msg {
                    ...
                    Message::Text(text) => {
                        process_text_msg(&chat_relay, &mut session, &text, conn_id, &mut name)
                            .await;
                    }
...
async fn process_text_msg(
    ...
    if msg.starts_with('/') {
        ...
        match cmd_args.next().unwrap() {
            "/list" => {
                log::info!("conn {conn_id}: listing rooms");

                let rooms = chat_relay.list_rooms().await;
            ...
            "/join" => match cmd_args.next() {
                Some(room) => {
                    log::info!("conn {conn_id}: joining room {room}");

                    chat_relay.join_room(conn_id, room).await;
            ...
            "/name" => match cmd_args.next() {
                Some(new_name) => {
                    log::info!("conn {conn_id}: setting name to: {new_name}");
        ...
        chat_relay.send_message(conn_id, msg).await
    }
}
